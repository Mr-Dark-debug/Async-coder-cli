...
import { EffectFlock } from "@async-coder/shared/util/effect-flock"
...
const managedConfigDir = process.env.ASYNC_CODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "async-coder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "async-coder.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "async-coder.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "async-coder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ASYNC_CODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ASYNC_CODER_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ASYNC_CODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ASYNC_CODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "async-coder.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .async-coder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".async-coder")
...
test("loads agents from .async-coder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".async-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .async-coder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".async-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .async-coder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".async-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("async-coder command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".async-coder", "command", "dup.md"),
        `---
description: async-coder version
---
from async-coder`,
      )
    },
...
test("does not try to install dependencies in read-only ASYNC_CODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ASYNC_CODER_CONFIG_DIR
  process.env.ASYNC_CODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ASYNC_CODER_CONFIG_DIR
    else process.env.ASYNC_CODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ASYNC_CODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .async-coder config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".async-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .async-coder config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "async-coder.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .async-coder config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "async-coder.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".async-coder")
      ...
      await Filesystem.write(
        path.join(local, "async-coder.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets ASYNC_CODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .async-coder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .async-coder directory config enables it
      const opencodeDir = path.join(dir, ".async-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "async-coder.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "async-coder.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.async-coder/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.async-coder/plugin/demo.ts", "file:///project/.async-coder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.async-coder/plugin/demo.ts"])
  })
...
describe("ASYNC_CODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"]
    process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .async-coder/ directories when flag is set", async () => {
    const originalEnv = process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"]
    process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .async-coder directory with a command
          const opencodeDir = path.join(dir, ".async-coder", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .async-coder should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ASYNC_CODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ASYNC_CODER_CONFIG_DIR"]
      process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ASYNC_CODER_CONFIG_DIR"]
      } else {
        process.env["ASYNC_CODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ASYNC_CODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ASYNC_CODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ASYNC_CODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ASYNC_CODER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from ASYNC_CODER_CONFIG_DIR, not project
...
describe("ASYNC_CODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ASYNC_CODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ASYNC_CODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ASYNC_CODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ASYNC_CODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ASYNC_CODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ASYNC_CODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ASYNC_CODER_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.async-coder.managed.test",
          PayloadType: "ai.async-coder.managed",
