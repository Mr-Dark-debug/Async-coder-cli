...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "async-coder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "async-coder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "async-coder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "async-coder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["async-coder.json", "async-coder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "async-coder.json"),
        path.join(tmp.path, "async-coder.jsonc"),
        path.join(project, "async-coder.json"),
        path.join(project, "async-coder.jsonc"),
      ])
    })
  })
