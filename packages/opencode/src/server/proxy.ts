...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-async-coder-directory")
  out.delete("x-async-coder-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-async-coder-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-async-coder-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
