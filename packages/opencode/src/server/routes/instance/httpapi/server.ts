...
const Headers = Schema.Struct({
  ...
  "x-async-coder-directory": Schema.optional(Schema.String),
})
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.ASYNC_CODER_SERVER_PASSWORD) return yield* effect

        const user = Flag.ASYNC_CODER_SERVER_USERNAME ?? "async-coder"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.ASYNC_CODER_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-async-coder-directory"] || process.cwd()
