...
import { Slug } from "@async-coder/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    ASYNC_CODER_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    ASYNC_CODER_WORKSPACE_ID: config.id,
    ASYNC_CODER_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.ASYNC_CODER_EXPERIMENTAL_WORKSPACES) return
