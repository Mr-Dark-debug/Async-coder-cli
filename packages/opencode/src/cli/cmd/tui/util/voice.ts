...
export async function transcribeAudio(opts: {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      model: process.env.ASYNC_CODER_VOICE_TRANSCRIBE_MODEL ?? "gpt-4o-mini-transcribe",
...
const VOICE_CONTROL_SYSTEM_PROMPT = `你是 async-coder（AI 编程助手）的语音输入助手。用户通过语音向输入框口述消息，这些消息将发送给 Code Agent 执行编程任务。用户可能使用中文或英文。
...
export async function processVoiceControl(opts: {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      model: process.env.ASYNC_CODER_VOICE_CONTROL_MODEL ?? "gpt-4o-mini",
      messages: [
