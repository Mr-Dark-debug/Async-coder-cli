...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall async-coder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall async-coder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g async-coder",
      pnpm: "pnpm uninstall -g async-coder",
      bun: "bun remove -g async-coder",
      // TODO(async-coder): uncomment when published to these channels
      // brew: "brew uninstall async-coder",
      // choco: "choco uninstall async-coder",
      // scoop: "scoop uninstall async-coder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".async-coder")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using async-coder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# async-coder") || content.includes(".async-coder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".async-coder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".async-coder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".async-coder"))
    ) {
      continue
    }
