 ...
 * TODO: lift to async-coder.json config (e.g. session.maxGoalReact).
 */
...
const OUTPUT_LENGTH_CONTINUATION_LIMIT = Flag.ASYNC_CODER_OUTPUT_LENGTH_CONTINUATION_LIMIT
const INVALID_OUTPUT_CONTINUATION_LIMIT = Flag.ASYNC_CODER_INVALID_OUTPUT_CONTINUATION_LIMIT

const log = Log.create({ service: "session.prompt" })

function isExtensionPath(filePath: string): boolean {
  return /\/\.async-coder\/(tools?|skills?|hooks?)\//.test(filePath)
}
    ...
    const runLoop: (sessionID: SessionID, agentID?: string, task_id?: string) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
      ...
      function* (sessionID: SessionID, agentID?: string, task_id?: string) {
        ...
        const publishAgentRequest = (phase: string, taskType: string) =>
          bus
            .publish(Metrics.AgentRequest, {
              ...
              phase,
              task_type: taskType,
              surface: Flag.ASYNC_CODER_CLIENT,
        ...
        while (true) {
          ...
          // (explore/general/etc) spawned via async-coder's shared-sessionID
