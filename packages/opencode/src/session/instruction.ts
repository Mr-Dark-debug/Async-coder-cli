...
const FILES = [
  "AGENTS.md",
  ...(Flag.ASYNC_CODER_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.ASYNC_CODER_CONFIG_DIR) {
    files.push(path.join(Flag.ASYNC_CODER_CONFIG_DIR, "AGENTS.md"))
  }
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.ASYNC_CODER_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no ASYNC_CODER_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.ASYNC_CODER_CONFIG_DIR, Flag.ASYNC_CODER_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
