...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".async-coder-project-id")
  const idFile = nodePath.join(mainGit, "async-coder-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .async-coder-project-id
  ...
  if (!existing.includes(".async-coder-project-id")) {
    await nodeFs.appendFile(excludeFile, "\n.async-coder-project-id\n")
  }
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.ASYNC_CODER_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        if (Flag.ASYNC_CODER_DISABLE_GIT) {
          return {
            id: ProjectID.global,
            worktree: directory,
            sandbox: directory,
        ...
        // turn, which has shipped as a bug for users opening async-coder in $HOME.
      ...
      if (Flag.ASYNC_CODER_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
