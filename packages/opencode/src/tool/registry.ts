...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@async-coder/plugin"
import z from "zod"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.ASYNC_CODER_CLIENT) || Flag.ASYNC_CODER_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.history,
            tool.task,
            ...(Flag.ASYNC_CODER_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      let filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          ...
          return input.providerID === ProviderID.opencode || Flag.ASYNC_CODER_ENABLE_EXA
        }
