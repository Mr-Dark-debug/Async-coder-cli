...
The `advisor` option configures the **Sage** second-opinion model used by the `consult` tool and `/consult` command. `variant` is optional and must be a reasoning level advertised by the selected model. Run `/sage-model` to configure or change these values interactively.
