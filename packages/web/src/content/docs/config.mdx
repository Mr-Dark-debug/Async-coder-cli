...
You can configure the providers and models you want to use in your OpenCode config through the `provider`, `model`, `small_model`, and `advisor` options.
...
The `advisor` option configures the **Sage** second-opinion model used by the `consult` tool and `/consult` command. `variant` is optional and must be a reasoning level advertised by the selected model. Run `/sage-model` to configure or change these values interactively. The picker can connect an unconfigured provider through its existing setup flow, validate and refresh supported model catalogues, and resume model selection automatically. Ollama and custom OpenAI-compatible endpoints are available through the local provider choices.

Sage stores only the selected provider/model identifier and optional reasoning variant. Provider API keys and OAuth credentials continue to use the normal provider credential store.
