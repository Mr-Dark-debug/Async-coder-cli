...
1. Start Ollama and make sure at least one model is installed (`ollama list`).
2. Run `/connect` and select **Ollama (local)**. OpenCode discovers installed models from the local Ollama API and saves the provider automatically.
3. Select the model immediately, or run `/models` later to change it.

If Ollama is installed but its local service is stopped, OpenCode shows an actionable connection error instead of adding an empty provider.
